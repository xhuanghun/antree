var YDataGrid = function(config){
		...
		var Handler = {
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					var idKey = dataGrid.idField || 'id'; //主键名称
 					...
 					antree.getById(Action.getId,data,function(result){
			...
			remove: function(callback){
				...
				if (Utils.checkSelect(records)){
					$.messager.confirm('提示','你确定要删除记录?',function(r){  
					    if (r){
					    	var idKey = dataGrid.idField || 'id'; //主键名称
					    	var  data = $("input[name='"+idKey+"']", Form.list ).fieldSerialize(); //序列化字段
					    	antree.deleteForm(Action.remove,data,function(result){
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
									callback(result);
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					antree.saveForm(Form.edit,function(data){
		...
		var initGrid = function(){
			var dataconfig = {
				...
				nowrap: false,
				fitColumns:dataGrid.fitColumns || false,
				autoRowHeight: false,
				striped: true,
		...
		this.initTbar = function(){
			...
			//查询页面授权的btnType
			antree.ajaxJson(_url,data,function(data){
				if(data.success){
					if(data.allType){
		...
		var initForm = function(){
			if(Form.search && Form.search[0]){
				...
				Form.search.find("input").keydown(function(callback){
		...
		/*
		 var initForm = function(callback){
			if(Form.search && Form.search[0]){
				//回调函数
				if(jQuery.isFunction(callback)){
					Form.search.find("#btn-search").click(callback);
				}
				else{
					Form.search.find("#btn-search").click(Events.search); //保存事件
				}
