$package('antree.sysMenu');

antree.sysMenu = function(){
	var _box = null;
	var _this = {
		...
		addLine: function(data){
			...
			$(".remove-btn",line).click(function(){
				antree.confirm('提示','你确定要删除记录?',function(r){
					if(r){
						_this.delLine(line);
					}
				})
  			...
  			dataGrid:{
  				title:'菜单列表',
	   			...
	   			columns:[[
					...
					{field:'rank',title:'排序',align:'right',width:80,sortable:true,align:'center'},
					{field:'url',title:'URL地址',width:220,sortable:true,align:'center'},
					{field:'createTime',title:'添加时间',width:200,sortable:true,align:'center'},
					{field:'updateTime',title:'修改时间',width:200,sortable:true,align:'center'},
					{field:'childMenus',title:'子菜单',width:120,align:'center',formatter:function(value,row,index){
						var html ="<a href='#' onclick='antree.sysMenu.toList("+row.id+")'>子菜单管理("+row.subCount+")</a>";
						return html;
					}}
				]],
				toolbar:[
		...
		init:function(){
			...
			$('#delAllLine_btn').click(function(){
				antree.confirm('提示','你确定要删除记录?',function(r){  //antree.confirm('Confirm','Are you sure you want to delete record?',function(r){
